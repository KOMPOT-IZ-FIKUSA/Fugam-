        ...
        public override void OnInspectorGUI()
        {
            ...
            if (tex && (target as ColorCorrectionLookup).basedOnTempTex != AssetDatabase.GetAssetPath(tex))
            {
                ...
                else if (GUILayout.Button("Convert and Apply"))
                {
                    ...
                    // Replaced the obsolete textureFormat with textureCompression
                    ...
                    if (doImport)
                    {
                        ...
                        // Set the textureCompression property instead of textureFormat
                        textureImporter.textureCompression = TextureImporterCompression.Uncompressed;

                        // You may want to adjust PlatformTextureSettings if needed for different platforms
                        var platformSettings = textureImporter.GetPlatformTextureSettings("Default");
                        platformSettings.overridden = true;  // Make sure the settings are applied to all platforms
                        platformSettings.compressionQuality = 0; // Lowest quality for uncompressed texture (like AutomaticTruecolor)
